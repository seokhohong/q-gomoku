...
pyspark --name "Pyspark Notebook" --num-executors 1 --conf spark.executor.memory='8g' --conf spark.dynamicAllocation.enabled=true \
--conf spark.shuffle.service.enabled=true --conf spark.dynamicAllocation.minExecutors=1 --conf spark.dynamicAllocation.maxExecutors=10 \
...
--conf spark.rpc.message.maxSize='2047' --jars=gs://hadoop-lib/bigquery/bigquery-connector-hadoop2-latest.jar --py-files src.zip