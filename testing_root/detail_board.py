...
class Board:
    ...
    def unmove(self):
        ...
        self.chain_length_memory = previous_chain
    ...
    def cache_rotations(self):
        indices = np.array(range(self.size ** 2)).reshape(self.size, self.size, 3)
        for matrix in [
                indices,
                indices.transpose(),
                np.rot90(indices),
    ...
    def compute_game_state(self):
        ...
        if last_move:
            ...
            # check win
            if self.chain_length(last_x, last_y, -1, 0) >= self.win_chain_length\
                    or self.chain_length(last_x, last_y, -1, 1) >= self.win_chain_length \
                    or self.chain_length(last_x, last_y, 1, 1) >= self.win_chain_length \
                    or self.chain_length(last_x, last_y, 0, 1) >= self.win_chain_length:
